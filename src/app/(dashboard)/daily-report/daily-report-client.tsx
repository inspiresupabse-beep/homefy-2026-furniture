...
import { ExternalLink, RefreshCw, Send, ClipboardList } from "lucide-react";

const METRIC_FIELDS: {
  key: keyof typeof EMPTY_DAILY_METRICS;
  label: string;
  icon: string;
  step?: string;
}[] = [
...
function mapReportRow(row: Record<string, unknown>): DailyStaffReport {
  return {
    id: String(row.id),
...
export default function DailyReportPageClient() {
  ...
  const loadReport = useCallback(async () => {
    ...
    const [{ data: profileData }, { data: leadsData }, { data: ordersData }, { data: reportData }] =
      await Promise.all([
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <PageHeader
        ...
        description="End-of-day follow-up summary — resets automatically each new day"
      />
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        ...
        {sentAt && (
          <p className="mt-1 text-xs text-emerald-700">
            Last sent: {new Date(sentAt).toLocaleString("en-IN")}
          </p>
        )}
      </div>
                ...
                <div key={key}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setMetrics((prev) => ({
                        ...prev,
                        [key]: step ? Number(e.target.value) || 0 : Math.max(0, parseInt(e.target.value, 10) || 0),
                      }))
                    }
                  />
                </div>
